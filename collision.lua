...
local function rect2circle (circle, rect)
	if circle.x + circle.r >= rect.x1 and circle.x - circle.r <= rect.x1 + rect.w and circle.y + circle.r
	>= y1 and circle.y - circle.r <= rect.y1 + rect.h then --х, у, r относятся к кругу, r - радиус.
		return true
	else
		return false
	end
